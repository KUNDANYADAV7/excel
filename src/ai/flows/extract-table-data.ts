...
4.  **Consistent Column Count (CRUCIAL):**
    *   First, analyze the entire image to determine the correct number of columns. This determination must be based on the clearest and most consistent visual column alignments observable throughout the table in the IMAGE.
...
5.  **Cell Content Integrity (ULTRA-CRITICAL - DO NOT SPLIT CELLS BASED ON INTERNAL SPACES):**
    ...
    *   This applies even if the text contains internal spaces (e.g., "GRINDELF S3Q 7RQ" for a combined City/Zipcode, "078 0729" for a telephone number, "October, 2" for a birthday, "911 33 56 4 C" for an ID, "MM/YY" for an expiry date).
...
6.  **Header Row Exclusion:** DO NOT include the header row from the image (e.g., the row containing "Cyname", "Surname", "Gender", etc.). Output data rows only.
